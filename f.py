...
def _load_faq_index() -> list[dict]:
    """Load entries safely with total protection against missing schema properties."""
...
# --------------------------------------------------------------------------
# Azure OpenAI Integration mapping your configs safely inside parameters
# --------------------------------------------------------------------------
llm = AzureChatOpenAI(
...
class AgentState(TypedDict):
    # Safe fallback: Using clear add_messages framework to allow graph nodes to control slicing
...
def agent_node(state: AgentState) -> dict:
    ...
    # 🟢 BULLETPROOF CONTEXT WINDOW MANAGING:
    # Instead of slicing indices randomly which breaks the conversational structure,
    # we scan the history backwards and keep only the latest full user query turn.
    # This prevents OpenAI API from seeing orphaned tool replies, completely stopping the crash!
    messages = []
...
def retrieve_node(state: AgentState) -> dict:
    ...
    # OVERWRITE DEBUGGER.TXT SESSIONS ON EVERY SINGLE QUESTION TURN
...
# Build execution graph pipeline properties safely
...
if __name__ == "__main__":
    ...
    # use_reloader=False blocks any runtime reloader looping bugs permanently!
