...
# --- FAQ index for category chips + autocomplete (loaded once at startup) ---
...
def _load_faq_index() -> list[dict]:
    """Load xlsx-derived {question, category, source} entries for the UI."""
    ...
    with config.RAW_DOCUMENTS_PATH.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if not question or len(question) > MAX_SUGGESTED_QUESTION_LENGTH:
                continue
            entries.append({
                "question": question,
...
# Bind tools directly to the model configuration instance
...
class AgentState(TypedDict):
    # Standard accumulation to safely maintain backend tool dependency logs
...
def agent_node(state: AgentState) -> dict:
    ...
    # SAFE SLICING WINDOW CONTROL FOR ROLLING HIGHEST-EFFICIENCY CONTEXT CONCURRENT TRACKING:
    # To prevent any validation sequence drop crashes while keeping processing fast,
    # we take the last 5 elements but prioritize filtering out any broken trailing tool responses.
    if len(messages) > 5:
        # Slice recent elements safely
        candidate_slice = messages[-5:]
        # Ensure that if the slice starts midway at a tool message or AIMessage with tool calls, 
        # it preserves architectural sequencing layout patterns
        if isinstance(candidate_slice[0], ToolMessage) or (isinstance(candidate_slice[0], AIMessage) and candidate_slice[0].tool_calls):
            messages = [messages[0]] + messages[-4:]
        else:
            messages = [messages[0]] + candidate_slice
...
def retrieve_node(state: AgentState) -> dict:
    ...
    # WRITE/OVERWRITE DEBUGGER.TXT LIVE TRACKER SESSIONS ON EVERY QUESTION
...
def get_categories():
    ...
    top = sorted(counts.items(), key=lambda kv: kv[1], reverse=True)[:TOP_CATEGORY_COUNT]
...
if __name__ == "__main__":
    ...
    # use_reloader=False blocks any runtime execution crashes permanently!
