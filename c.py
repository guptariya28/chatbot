...
llm = AzureChatOpenAI(
    ...
    # --- YEH DONO LINES APNE BRACKET KE ANDAR HONGIE (Line 74 se pehle) ---
...
# --------------------------------------------------------------------------
# Custom History Reducer (Limits history state to the last 5 messages)
# --------------------------------------------------------------------------
def limit_chat_history(left: list, right: list) -> list:
    """Combines message state lists and strictly retains only the last 5 messages."""
...
class AgentState(TypedDict):
    # Enforces the 5-message context window limit globally across the graph
    messages: Annotated[Sequence[BaseMessage], limit_chat_history]
    is_direct_faq: bool
    final_payload: dict 
...
def retrieve_node(state: AgentState) -> dict:
    ...
    # CRUCIAL REQUIREMENT: OVERWRITE DEBUGGER.TXT ON EVERY QUESTION WITH RETRIEVED BLOCKS
...
# --------------------------------------------------------------------------
# Flask Endpoints Integration (Optimized & Cleaned)
# --------------------------------------------------------------------------
@app.route("/")
def index():
...
def get_categories():
    ...
    for entry in FAQ_INDEX:
        # Normalize category names to prevent case mismatches or empty values
    ...
    # Sort and extract the top configuration categories dynamically
...
def get_category_questions(category: str):
    # Dynamic case-insensitive sub-category match logic
...
def ask():
    ...
    try:
        # Fire conversational step invocation via LangGraph checkpointer thread tracking
